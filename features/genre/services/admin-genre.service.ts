...
import type {
  ...
  GenreQuery,
  UpdateGenreRequest,
} from "../types/admin-genre";
import { GenreSchema } from "../types/admin-genre";
...
export class GenreService {
  ...
  static async getById(id: string): Promise<Genre> {
    try {
      ...
      if (!isSuccessResponse(response)) {
        throw new ApiError(response.message || "Failed to fetch genre");
      }
  ...
  static async update(id: string, data: UpdateGenreRequest): Promise<Genre> {
    try {
      ...
      if (!isSuccessResponse(response)) {
        throw new ApiError(response.message || "Failed to update genre");
      }
  ...
  static async delete(id: string): Promise<void> {
    try {
      ...
      if (!isSuccessResponse(response)) {
        throw new ApiError(response.message || "Failed to delete genre");
      }
    } catch (error) {
      if (error instanceof ApiError) {
        throw error;
