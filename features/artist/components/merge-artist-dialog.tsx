...
const formSchema = z.object({
  targetId: z.string().min(1, "Vui lòng chọn hoạ sĩ đích"),
  sourceIds: z.array(z.string()).min(1, "Vui lòng chọn ít nhất 1 hoạ sĩ nguồn"),
});

interface MergeArtistDialogProps {
  open: boolean;
...
export function MergeArtistDialog({
  ...
  } = useArtists(targetSearch);

  // Hook for Source Artists
  const {
    data: sourceArtists,
  ...
  // Prepare options
  const targetOptions = targetArtists.map((a) => ({
    label: a.name,
    value: a.id,
  }));
  const sourceOptions = sourceArtists
    .map((a) => ({
      label: a.name,
      value: a.id,
    }))
  ...
  const entityName = tEntities("artist");
  const entityPlural = tEntities("artists");

  function onSubmit(values: z.infer<typeof formSchema>) {
    startTransition(async () => {
      try {
        if (step === 1) {
          // Verify sources not empty and not same as target
          const data = await previewMergeArtist({
