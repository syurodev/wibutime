...
export class ContentService {
   ...
   * @example
   * ```ts
   * const result = await ContentService.getLibraryPaginated({ type: 'anime', page: 1, sort: 'title' });
   * ```
   */
  static async getLibraryPaginated(options?: {
    ...
    const { type = "all", page = 1, limit = 20, sort = "recent" } = options || {};
    await mockDelay();
    ...
    switch (sort) {
      case "title":
        sortedSeries = filteredSeries.toSorted((a, b) => a.title.localeCompare(b.title));
        break;
      case "updated":
        ...
        break;
      case "rating":
        sortedSeries = filteredSeries.toSorted((a, b) => b.favorites - a.favorites);
        break;
      case "recent":
      default:
