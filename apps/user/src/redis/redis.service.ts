...
export class RedisService {
  ...
  async getAccessTokenPayload(userId: number, token: string) {
    const [key] = await this.redis.keys(REDIS_KEY.ACCESS_TOKEN(userId, token));
  ...
  async existsAccessToken(token: string) {
    const exists = await this.redis.keys(REDIS_KEY.ACCESS_TOKEN('*', token));
  ...
  async removeAccessToken(userId: number) {
    const [key] = await this.redis.keys(REDIS_KEY.ACCESS_TOKEN(userId, '*'));
    await this.redis.del(key);
  }

  async removeSpecificAccessToken(userId: number, token: string) {
    const key = REDIS_KEY.ACCESS_TOKEN(userId, token);
    await this.redis.del(key);
  }
  ...
  async removeListAccessToken(userIds: number[]) {
    ...
    for (const userId of userIds) {
      const keys = await this.redis.keys(REDIS_KEY.ACCESS_TOKEN(userId, '*'));
      if (keys.length > 0) {
        multi.del(...keys);
      }
    }
  ...
  async setCache(value: any, key: string, prefix?: string, expiresIn?: number) {
