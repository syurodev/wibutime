...
export default async function WorkspaceNovelsPage({ params }: Props) {
  const { tenantId } = await params;

  // TODO: Fetch real novels from API with owner_type='tenant' and owner_id=tenantId
  const novels: any[] = [];
    ...
    <Container maxWidth="2xl" className="py-8 space-y-8">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-3xl font-bold">Novels của Team</h1>
            <p className="text-muted-foreground mt-1">
              Quản lý novels thuộc sở hữu của team
            </p>
          </div>
        </div>
        <Button asChild>
          <Link href={`/workspace/${tenantId}/novels/create`}>
