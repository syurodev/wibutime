...
import { LibraryFilters, type SortOption } from "@/components/library/LibraryFilters";
...
async function LibraryContent({
	...
	const { items } = await ContentService.getLibraryPaginated({
		type,
		page,
		limit: 15,
		sort,
...
export default async function LibraryPage({ searchParams }: LibraryPageProps) {
	...
	const { totalPages, currentPage, totalItems } =
		await ContentService.getLibraryPaginated({
			type: selectedType,
			page,
			limit: 15,
			...
			<Container maxWidth="xl" bottomSpacing>
				...
				{hasContent ? (
					<>
						...
						>
							<LibraryContent type={selectedType} page={page} sort={selectedSort} />
						</Suspense>
